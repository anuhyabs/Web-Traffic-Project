...
```{r}
wtd_zh_train <- window(wtd_zh_ts, end = c(100,1))
wtd_zh_test <- window(wtd_zh_ts, start = c(100,2))
...
```
The plot shows one major peak around the 140th day (approx.) and a small peak arounf the 280th . Representative of quarterly seasonality? 
...
```{r}
wtd_zh_ts.d1 <- diff(wtd_zh_train, 1)
plot(wtd_zh_ts.d1,
...
```{r}
wtd_zh_ts.d2 <- diff(wtd_zh_ts.d1, lag = 7)
plot(wtd_zh_ts.d2,
     main = "Web Traffic Analysis: Chinese",
     ylab = "Chinese Pages", type = 'l')
kpss.test(wtd_zh_ts.d2)
acf2(wtd_zh_ts.d2)
```
...
p = 1/5
d = 0
I would apply SARIMA(1,0,3)(0,1,3)[7], SARIMA(5,0,3)(0,1,3)[7] and run auto ARIMA.

ARIMA Modeling:

```{r}
wtd_zh_sm1 <- sarima(wtd_zh_train, S = 7, diff = 1,
                     p = 4, d = 1, q = 2,
                     P = 0, D = 1, Q = 2)
wtd_zh_sm1
```
...
```{r}
wtd_zh_sm1_for <- sarima.for(wtd_zh_train,n.ahead = 109,S = 7,
                     p = 4, d = 1, q = 2,
                     P = 0, D = 1, Q = 2)
...
```{r}
wtd_fr_train <- window(wtd_fr_ts, end = c(100,1))
wtd_fr_test <- window(wtd_fr_ts, start = c(100,2))
...
```{r}
wtd_fr_ts.d1 <- diff(wtd_fr_train, 1)
plot(wtd_fr_ts.d1,
...
```{r}
wtd_fr_sm1_for <- sarima.for(wtd_fr_train,n.ahead = 109,S = 7,
                     p = 0, d = 0, q = 4,
                     P = 0, D = 1, Q = 2)
...
```{r}
wtd_en_train <- window(wtd_en_ts, end = c(100,1))
wtd_en_test <- window(wtd_en_ts, start = c(100,2))
...
```{r}
wtd_en_sm1_for <- sarima.for(wtd_en_train,n.ahead = 109,S = 7,
                      p = 1, d = 0, q = 0,
                     P = 2, D = 1, Q = 0)
...
```{r}
wtd_ru_train <- window(wtd_ru_ts, end = c(100,1))
wtd_ru_test <- window(wtd_ru_ts, start = c(100,2))
...
```
There is a weekly seasonality that can be seen in the spectral analysis. There are multiple peaks in the begginning of the plot followed by a peak at around the 60th day ans a small peak around the 140th day. There are no other significant peaks.
...
```{r}
wtd_ru_sm1_for <- sarima.for(wtd_ru_train,n.ahead = 109,S = 7,
                     p = 2, d = 1, q = 0,
                     P = 0, D = 0, Q = 2)
...
```{r}
wtd_de_train <- window(wtd_de_ts, end = c(100,1))
wtd_de_test <- window(wtd_de_ts, start = c(100,2))
...
```
There is a weekly seasonality that can be seen in the spectral analysis. There are also small peaks at the beggining of the plot followed by three significant peaks around 140th, 280th and 420th days (approx). This may signify some kind of quarterly seasonality.
...
```{r}
wtd_de_sm1_for <- sarima.for(wtd_de_train,n.ahead = 109,S = 7,
                     p = 4, d = 0, q = 0,
                     P = 0, D = 1, Q = 1)
...
```{r}
wtd_ja_train <- window(wtd_ja_ts, end = c(100,1))
wtd_ja_test <- window(wtd_ja_ts, start = c(100,2))
...
Forecasting:
```{r}
wtd_ja_sm1_for <- sarima.for(wtd_ja_train,n.ahead = 109,S = 7,
                      p = 3, d = 0, q = 1,
                     P = 0, D = 1, Q = 2)
...
```{r}
wtd_es_train <- window(wtd_es_ts, end = c(100,1))
wtd_es_test <- window(wtd_es_ts, start = c(100,2))
...
Forecasting:
```{r}
wtd_es_sm1_for <- sarima.for(wtd_es_train,n.ahead = 109,S = 7,
                     p = 1, d = 0, q = 2,
                     P = 2, D = 1, Q = 0)
